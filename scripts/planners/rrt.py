...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        while n <= max_iters and not success:
            ...
            x_near = self.find_nearest(V[range(n), :], x_rand)
    ...
    def shortcut_path(self):
        ...
        while not success:
            ...
            while ii != len(self.path) - 1:
                if self.is_free_motion(self.obstacles, self.path[ii - 1], self.path[ii + 1]):
                    self.path.pop(ii)
                    success = False
                else:
                    ii += 1
...
class GeometricRRT(RRT):
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        if dist <= eps:
            ...
            direction = x2 - x1
            newState = x1 + direction * eps / dist
        return newState
        ########## Code ends here ##########
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        for ii in range(len(V)):
            path = dubins.shortest_path(V[ii], x, self.turning_radius)
